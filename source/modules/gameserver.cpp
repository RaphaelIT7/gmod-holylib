...
LUA_FUNCTION_STATIC(CNetChan_Transmit)
{
	...
	//g_bFreeSubChannels = bFreeSubChannels; // ... This is OUR FKING CHANNEL. We don't detour it.
	LUA->PushBool(pNetChannel->Transmit(bOnlyReliable));
	//g_bFreeSubChannels = false;
...
{
	int ret = detour_CNetChan_SendDatagram.GetTrampoline<Symbols::CNetChan_SendDatagram>()(chan, datagram);
