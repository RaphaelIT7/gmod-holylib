...
class CLagCompensationModule : public IModule
{
public:
	virtual void Init(CreateInterfaceFn* appfn, CreateInterfaceFn* gamefn) OVERRIDE;
	virtual void InitDetour(bool bPreServer) OVERRIDE;
	virtual const char* Name() { return "lagcompensation"; };
	virtual int Compatibility() { return LINUX32; };
};

static CLagCompensationModule g_pLagCompensationModule;
IModule* pLagCompensationModule = &g_pLagCompensationModule;
...
//ConVar* sv_unlag_fixstuck;
//ConVar* sv_maxunlag;
//ConVar* sv_unlag;
//ConVar* sv_unlag_debug;
ConVar* sv_lagcompensationforcerestore;
ConVar* sv_lagcompensateself;
...
Symbols::UTIL_TraceEntity1 func_UTIL_TraceEntity1 = NULL;
...
{
	if (!func_UTIL_TraceEntity1)
		return;

	func_UTIL_TraceEntity1(pEntity, vecAbsStart, vecAbsEnd, mask, pIgnore, nCollisionGroup, ptr);
}

Symbols::UTIL_TraceEntity2 func_UTIL_TraceEntity2 = NULL;
void UTIL_TraceEntity( CBaseEntity *pEntity, const Vector &vecAbsStart, const Vector &vecAbsEnd, unsigned int mask, trace_t *ptr )
{
	if (!func_UTIL_TraceEntity2)
		return;

	func_UTIL_TraceEntity2(pEntity, vecAbsStart, vecAbsEnd, mask, ptr);
}
...
void CLagCompensationModule::Init(CreateInterfaceFn* appfn, CreateInterfaceFn* gamefn)
{
	//sv_lagflushbonecache = g_pCVar->FindVar("sv_lagflushbonecache");
	//sv_unlag_fixstuck = g_pCVar->FindVar("sv_unlag_fixstuck");
	//sv_maxunlag = g_pCVar->FindVar("sv_maxunlag");
	//sv_unlag = g_pCVar->FindVar("sv_unlag");
	//sv_unlag_debug = g_pCVar->FindVar("sv_unlag_debug");
	//sv_lagcompensation_teleport_dist = g_pCVar->FindVar("sv_lagcompensation_teleport_dist");
	sv_lagcompensationforcerestore = g_pCVar->FindVar("sv_lagcompensationforcerestore");
	sv_lagcompensateself = g_pCVar->FindVar("sv_lagcompensateself");

	g_pSharedChangeInfo = Util::engineserver->GetSharedEdictChangeInfo();
}

void CLagCompensationModule::InitDetour(bool bPreServer)
{
	if (bPreServer)
		return;
	...
	func_UTIL_TraceEntity1 = (Symbols::UTIL_TraceEntity1)Detour::GetFunction(server_loader.GetModule(), Symbols::UTIL_TraceEntity1Sym);
	Detour::CheckFunction((void*)func_UTIL_TraceEntity1, "UTIL_TraceEntity1");

	func_UTIL_TraceEntity2 = (Symbols::UTIL_TraceEntity2)Detour::GetFunction(server_loader.GetModule(), Symbols::UTIL_TraceEntity2Sym);
	Detour::CheckFunction((void*)func_UTIL_TraceEntity2, "UTIL_TraceEntity2");
}