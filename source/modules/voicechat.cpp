...
static ConVar voicechat_savedecompressed("holylib_voicechat_savedecompressedd", "0", FCVAR_ARCHIVE, "If enabled the VoiceData will store the decompressed data improving quality when applying multiple effects since it no longer needs to decompress the data each time");
	...
	inline char* GetData()
	{
		if (bDecompressedChanged && pDecompressedData)
		{
			...
			if (bytes == -1)
			{
				...
				{
					Msg(PROJECT_NAME " - voicechat: Failed to compress data in VoiceData::GetData!\n");
				}
