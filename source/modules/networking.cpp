...
{
	return g_pShouldPrevent[ply->entindex()-1].IsBitSet(ent->entindex());
}

static void CleaupSetPreventTransmit(CBaseEntity* ent)
{
	int entIndex = ent->entindex();
	if (!ent->IsPlayer())
	{
...
{
	int plyIndex = pPly->entindex() - 1;
	int entIndex = pEnt->entindex();
	if (bPreventTransmit) {
...
bool New_CServerGameEnts_CheckTransmit(IServerGameEnts* gameents, CCheckTransmitInfo *pInfo, const unsigned short *pEdictIndices, int nEdicts)
{
	...
	const int clientArea = engine->GetArea((pRecipientPlayer->GetViewEntity() != NULL) ? pRecipientPlayer->GetViewEntity()->GetAbsOrigin() : pRecipientEntity->GetAbsOrigin());
	...
	CBitVec<MAX_EDICTS>& pPreventTransmitVec = g_pShouldPrevent[clientIndex];
	...
	for ( int i=0; i < nEdicts; i++ )
	{
		...
		if ( pPreventTransmitVec.Get(iEdict) ) // Implements gmod's SetPreventTransmit but far faster.
			continue;
	...
	if (bFastPath && !bIsHLTV && !(pRecipientPlayer->GetObserverMode() == OBS_MODE_IN_EYE && pRecipientPlayer->GetObserverTarget()))
	{
		...
		if (func_CBasePlayer_GetViewModel)
		{
			for (int iViewModel=0; iViewModel<MAX_VIEWMODELS; ++iViewModel)
			{
				...
				if (pViewModel)
				{
					pInfo->m_pTransmitEdict->Clear(pViewModel->entindex());

					for (int iScreen=0; iScreen < pViewModel->m_hScreens.Count(); ++iScreen)
					{
						CVGuiScreen* pScreen = pViewModel->m_hScreens[iScreen].Get();
						if (pScreen)
						{
							pInfo->m_pTransmitEdict->Clear(pScreen->entindex());
						}
					}
				}
			}
...
void PackEntities_Normal(int clientCount, CGameClient **clients, CFrameSnapshot *snapshot)
{
	...
	CUtlVectorFixed< PackWork_t, MAX_EDICTS > workItems(0, snapshot->m_nValidEntities);
