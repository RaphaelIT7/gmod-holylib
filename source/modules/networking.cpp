...
static bool g_pShouldPrevent[MAX_EDICTS][MAX_PLAYERS] = {0};
...
#endif
ConVar networking_fastpath("holylib_networking_fastpath", "0", 0, "Experimental - If two players are in the same area, then it will reuse the transmit state of the first calculated player saving a lot of time");
void New_CServerGameEnts_CheckTransmit(IServerGameEnts* gameents, CCheckTransmitInfo *pInfo, const unsigned short *pEdictIndices, int nEdicts)
{
	...
	const int clientArea = engine->GetArea(pRecipientEntity->GetAbsOrigin());
	...
	if (bFirstTransmit)
	{
		...
		if (bFastPath)
		{
			for (int iOtherClient = 0; iOtherClient<MAX_PLAYERS; ++iOtherClient)
			{
				...
				pRecipientEntity->SetTransmit(pInfo, true); // Call this for our player

				for (int i=0; i < nEdicts; ++i)
				{
					int iEdict = pEdictIndices[i];

					if (g_pShouldPrevent[iEdict][clientIndex])
					{
				...
				return; // fast route when players are in the same area, we can save a tone of calculation hopefully without breaking anything.
			}
		}
	}
	...
	for ( int i=0; i < nEdicts; i++ )
	{
		...
		if ( g_pShouldPrevent[iEdict][clientIndex] ) // Implements gmod's SetPreventTransmit but hopefully faster.
			continue;
