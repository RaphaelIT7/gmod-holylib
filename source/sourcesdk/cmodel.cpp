...
const byte *CM_Vis( byte *dest, int destlen, int cluster, int visType )
{
	...
	if ( cluster == -1 )
	{
		...
		if ( len > destlen )
		{
			Error( "CM_Vis:  buffer not big enough (%i but need %i)\n",
				destlen, len );
		}
...
void CM_SetAreaPortalState( int portalnum, int isOpen )
{
	...
	{
		Error( "portalnum > numareaportals");
	}
...
bool	CM_AreasConnected (int area1, int area2)
{
	...
	{
		Error( "area(1==%i, 2==%i) >= numareas (%i):  Check if engine->ResetPVS() was called from ClientSetupVisibility", area1, area2,  pBSPData->numareas );
	}
...
int CM_WriteAreaBits ( byte *buffer, int buflen, int area )
{
	...
	else
	{
		if ( buflen < 32 )
		{
			Error( "CM_WriteAreaBits with buffer size < 32\n" );
		}
...
int	CM_BoxVisible( const Vector& mins, const Vector& maxs, const byte *visbits, int vissize )
{
	...
	for ( int i = 0; i < count; i++ )
	{
		...
		{
			Error( "CM_BoxVisible:  cluster %i, offset %i out of bounds %i\n", cluster, offset, vissize );
		}
