...
namespace GarrysMod::Lua
{
	...
	public:
		...
		virtual void FindScripts(const std::string&, const std::string&, std::vector<GarrysMod::Lua::LuaFindResult>&) = 0;            // x86: 0x3C, x64: 0x78
	...
	static inline void FindScriptsSafe(ILuaShared* shared, const std::string& pattern, const std::string& pathId, std::vector<GarrysMod::Lua::LuaFindResult>& results) {
		if (!shared) return;
		...
		shared->FindScripts(pattern, pathId, results);
#endif
	}
}
#endif