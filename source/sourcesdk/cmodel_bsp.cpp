...
#include "module.h" // HolyLib include
...
void CollisionBSPData_LinkPhysics( void )
{
	...
	if( !physprop )
	{
		...
		physprop = (IPhysicsSurfaceProps*)g_pModuleManager.GetAppFactory()(VPHYSICS_SURFACEPROPS_INTERFACE_VERSION, NULL);
		physcollision = (IPhysicsCollision*)g_pModuleManager.GetAppFactory()(VPHYSICS_COLLISION_INTERFACE_VERSION, NULL);

		if ( !physprop || !physcollision )
		{
			Error( "CollisionBSPData_PreLoad: Can't link physics" );
		}
	}
}
...
void CollisionBSPData_LoadTextures( CCollisionBSPData *pBSPData )
{
	...
	if( lhStringTable.LumpSize() % sizeof( int ) )
		Error( "CMod_LoadTextures: funny lump size");
	...
	if (count < 1)
	{
		Error( "Map with no textures");
	}
	if (count > MAX_MAP_TEXDATA)
	{
		Error( "Map has too many textures");
	}
									...
									CUtlVector<unsigned short> &map_texinfo )
{
	...
	if (lh.LumpSize() % sizeof(*in))
		Error( "CollisionBSPData_LoadTexinfo: funny lump size");
	count = lh.LumpSize() / sizeof(*in);
	if (count < 1)
		Error( "Map with no texinfo");
...
void CollisionBSPData_LoadLeafs_Version_0( CCollisionBSPData *pBSPData, CMapLoadHelper &lh )
{
	...
	if (lh.LumpSize() % sizeof(*in))
	{
		Error( "CollisionBSPData_LoadLeafs: funny lump size");
	}
	...
	{
		Error( "Map leaf 0 is not CONTENTS_SOLID");
	}
...
void CollisionBSPData_LoadLeafBrushes( CCollisionBSPData *pBSPData )
{
	...
	if (lh.LumpSize() % sizeof(*in))
	{
		Error( "CMod_LoadLeafBrushes: funny lump size");
	}
	...
	{
		Error( "Map has too many leafbrushes");
	}
...
void CollisionBSPData_LoadPlanes( CCollisionBSPData *pBSPData )
{
	...
	if (lh.LumpSize() % sizeof(*in))
	{
		Error( "CollisionBSPData_LoadPlanes: funny lump size");
	}
...
void CollisionBSPData_LoadBrushes( CCollisionBSPData *pBSPData )
{
	...
	if (lh.LumpSize() % sizeof(*in))
	{
		Error( "CMod_LoadBrushes: funny lump size");
	}

	count = lh.LumpSize() / sizeof(*in);
	if (count > MAX_MAP_BRUSHES)
	{
		Error( "Map has too many brushes");
	}
...
void CollisionBSPData_LoadBrushSides( CCollisionBSPData *pBSPData, CUtlVector<unsigned short> &map_texinfo )
{
	...
	if (lh.LumpSize() % sizeof(*in))
	{
		Error( "CMod_LoadBrushSides: funny lump size");
	}
...
void CollisionBSPData_LoadSubmodels( CCollisionBSPData *pBSPData )
{
	...
	if (lh.LumpSize() % sizeof(*in))
		Error("CMod_LoadSubmodels: funny lump size");
	count = lh.LumpSize() / sizeof(*in);

	if (count < 1)
		Error( "Map with no models" );
	if (count > MAX_MAP_MODELS)
		Error( "Map has too many models" );
...
void CollisionBSPData_LoadNodes( CCollisionBSPData *pBSPData )
{
	...
	if (lh.LumpSize() % sizeof(*in))
		Error( "CollisionBSPData_LoadNodes: funny lump size");
	count = lh.LumpSize() / sizeof(*in);

	if (count < 1)
		Error( "Map has no nodes");
	if (count > MAX_MAP_NODES)
		Error( "Map has too many nodes");
...
void CollisionBSPData_LoadAreas( CCollisionBSPData *pBSPData )
{
	...
	if (lh.LumpSize() % sizeof(*in))
	{
		Error( "CMod_LoadAreas: funny lump size");
	}

	count = lh.LumpSize() / sizeof(*in);
	if (count > MAX_MAP_AREAS)
	{
		Error( "Map has too many areas");
	}
...
void CollisionBSPData_LoadAreaPortals( CCollisionBSPData *pBSPData )
{
	...
	if (lh.LumpSize() % sizeof(*in))
	{
		Error( "CMod_LoadAreaPortals: funny lump size");
	}
		   
	count = lh.LumpSize() / sizeof(*in);
	if (count > MAX_MAP_AREAPORTALS)
	{
		Error( "Map has too many area portals");
	}
...
void CollisionBSPData_LoadDispInfo( CCollisionBSPData *pBSPData )
{
	...
	if (coreDispCount > 1)
		coreDispCount = 1; // Fk all other ones :<
	...
	if ( lhv.LumpSize() % sizeof( dvertex_t ) )
		Error( "CMod_LoadDispInfo: bad vertex lump size!" );
    ...
    if ( lhe.LumpSize() % sizeof( dedge_t ) )
        Error( "CMod_LoadDispInfo: bad edge lump size!" );
    ...
    if ( lhs.LumpSize() % sizeof( int ) )
        Error( "CMod_LoadDispInfo: bad surf edge lump size!" );
    ...
    if ( lhf.LumpSize() % sizeof( dface_t ) )
        Error( "CMod_LoadDispInfo: bad face lump size!" );
