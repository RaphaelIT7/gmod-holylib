...
bool CGameEventManager::FireEventIntern( IGameEvent *event, bool bServerOnly, bool bClientOnly )
{
	...
	ConVarRef net_showevents("net_showevents");
	if ( net_showevents.GetInt() > 0 )
	{
		...
		{
			ConMsg( "Server event \"%s\", Tick %i:\n", descriptor->name, 0 ); // We currently don't have a good way to get the tickcount without bringing CGlobalVars into here :/
			ConPrintEvent( event );
		}
	}
...
int CGameEventManager::LoadEventsFromFile( const char * filename )
{
	...
	if  ( !key->LoadFromFile( g_pFullFileSystem, filename, "GAME" ) )
		return false;
