...
public:
	...
	bool	CompareClassBAdr (const netadrnew_s &a) const;
	bool	CompareClassCAdr (const netadrnew_s &a) const;
	...
	bool operator==(const netadrnew_s &netadr) const {return ( CompareAdr( netadr ) );}
	bool operator!=(const netadrnew_s &netadr) const {return !( CompareAdr( netadr ) );}
	bool operator<(const netadrnew_s &netadr) const;

public: // SteamID stuff.
	const CSteamID &GetSteamID() const { return m_SteamID; };
	void SetFromSteamID(CSteamID& steamID) {
		type = NA_STEAM;
		m_SteamID = steamID;
	};
...
class CUtlNetAdrRender_New
{
public:
	CUtlNetAdrRender_New( const netadrnew_s &obj, bool bBaseOnly = false )
	{
		obj.ToString( m_rgchString, sizeof(m_rgchString), bBaseOnly );
	}

	CUtlNetAdrRender_New( uint32 unIP )
	{
		netadrnew_s addr( unIP, 0 );
		addr.ToString( m_rgchString, sizeof(m_rgchString), true );
	}

	CUtlNetAdrRender_New( uint32 unIP, uint16 unPort )
	{
		netadrnew_s addr( unIP, unPort );
		addr.ToString( m_rgchString, sizeof(m_rgchString), false );
	}

	CUtlNetAdrRender_New( const struct sockaddr &s )
	{
		netadrnew_s addr;
