return {
    groupName = "INetworkStringTable:GetEntryBits",
    cases = {
        {
            name = "Function exists on meta table",
            when = HolyLib_IsModuleEnabled("stringtable"),
            func = function()
                expect( FindMetaTable("INetworkStringTable").GetEntryBits ).to.beA( "function" )
            end
        },
        {
        ...
        {
            name = "Matches log2 of the maxentries given at creation",
            when = HolyLib_IsModuleEnabled("stringtable"),
            func = function()
                ...
                expect( table:GetEntryBits() ).to.equal( 12 )
        ...
        },
        {
            ...
            when = HolyLib_IsModuleEnabled("stringtable"),
            func = function()
                expect( FindMetaTable("INetworkStringTable").GetEntryBits, 5 ).to.errWith( "bad argument #1 to '?' (Tried to use something that wasn't a INetworkStringTable!)" )
            end
        },
    }
}
