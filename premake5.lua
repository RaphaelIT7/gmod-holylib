...
CreateWorkspace({name = "holylib", abi_compatible = false})
	...
	CreateProject({serverside = true, manual_files = false, gmprefix = true})
		kind "SharedLib"
		symbols "On"
	...
	CreateProject({name = "ghostinj-dll", serverside = true, source_path = "ghostinj-dll", manual_files = false})
        kind "SharedLib"
        symbols "On"
        -- Remove some or all of these includes if they're not needed
        --IncludeHelpersExtended()
